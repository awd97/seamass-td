	...
	std::vector<double>& sts, std::vector< std::vector<double> >& mzs, std::vector< std::vector<double> >& intensities)
{
	...
	for (ii i = 0, j = 0; j < (ii)intensities.size(); j++)
	{
		...
		{
			js[i] = j;
			is[i + 1] = is[i] + (ii)intensities[j].size();
			i++;
		}
	}

	vector<fp> gs(is.back());
	#pragma omp parallel for
	for (ii j = 0; j < nj; j++)
	{
	...
	cout << "DataMS id=\"" << id << "\" config_id=\"" << config_id << "\" instrument_type=" << instrument_type;
	cout << " nspectra=" << js.size() << "/" << mzs.size() << " g=" << g << " (" << fixed << setprecision(2) << g.mem() / 1024.0 / 1024.0 << "Mb)" << endl;
}
